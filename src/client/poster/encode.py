...
class MultipartParam(object):
    ...
    def encode_hdr(self, boundary):
        ...
        boundary2 = "zzXXzzYYzzXXzzQQ"

        headers = ["--%s\r\n" % boundary2]

        if self.filename:
            disposition = 'form-data; name="%s"; filename="%s"' % (boundary,
                    self.filename)
        else:
            disposition = 'form-data; name="%s"' % boundary
...
def get_headers(name, params, boundary):
    """Returns a dictionary with Content-Type and Content-Length headers
    for the multipart/form-data encoding of ``params``."""
    print "CREATING HEADERS FOR FILE: "+ name
    headers = {}
    boundary = urllib.quote_plus(boundary)
    headers['Content-Type'] = "multipart/form-data; boundary=%s" % boundary
    headers['Content-Length'] = str(get_body_size(params, boundary))
    headers['User-Agent'] = "WhatsApp/1.2 S40Version/microedition.platform"
    headers[boundary] = "https://mms.whatsapp.net/client/iphone/upload.php"
    headers['Connection'] = "close"
    headers['Host'] = "mms.whatsapp.net"
    return headers
...
def multipart_encode(filename, params, boundary=None, cb=None):
    ...
    """
    if boundary is None:
        boundary = gen_boundary(filename)
    else:
        boundary = urllib.quote_plus(boundary)

    headers = get_headers(filename, params, boundary)
